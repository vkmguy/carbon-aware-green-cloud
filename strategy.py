...
class AdHocStrategy(Strategy):
    ...
    def __init__(self, node: Node, ci: pd.DataFrame, interval: int, forecast: Union[str, int] = None,
    ...
    def _forecast_steps(self, timeline_idx: int, duration_steps: int) -> int:
        ...
        if self.forecast == "next_workday":
            ...
            if (weekday == 0 and time > 9) or weekday == 1 or weekday == 2 or (
                    weekday == 3 and time <= 9):  # Shift to Thu
                finish_timestamp = oft.replace(hour=9, minute=0, second=0) + timedelta(days=(7 - weekday + 3) % 7)
            if (weekday == 3 and time > 9) or weekday == 4 or weekday == 5 or weekday == 6 or (
                    weekday == 0 and time <= 9):  # Shift to Mo
        ...
        elif self.forecast == "weekend":
                if weekday == 0:  # monday
                    finish_timestamp = oft.replace(hour=23, minute=59, second=59) + timedelta(days=6)
                elif weekday == 1:  # tuesday
                    finish_timestamp = oft.replace(hour=23, minute=59, second=59) + timedelta(days=5)
                elif weekday == 2:  # wednesday
                    finish_timestamp = oft.replace(hour=23, minute=59, second=59) + timedelta(days=4)
                elif weekday == 3:  # thursday
                    finish_timestamp = oft.replace(hour=23, minute=59, second=59) + timedelta(days=3)
                elif weekday == 4:  # friday
                    finish_timestamp = oft.replace(hour=23, minute=59, second=59) + timedelta(days=2)
                else:
                    finish_timestamp = oft
        else:
...
#     """A strategy for ad hoc workloads on weekends, i.e. workloads that can be shifted into the future (postponed)."""
#
#     def __init__(self, node: Node, ci: pd.DataFrame, interval: int, forecast: Union[str, int] = None,
#                  interruptible: bool = False):
#         super().__init__(node)
...
#         if oft.minute == 30:
#             time += 0.5
#         if self.forecast == "weekend":
#             if weekday == 0 and 9 < time < 17:  # monday
#                 finish_timestamp = oft.replace(hour=0, minute=0, second=0) + timedelta(days=5)
#             elif weekday == 1 and 9 < time < 17:  # tuesday
#                 finish_timestamp = oft.replace(hour=0, minute=0, second=0) + timedelta(days=4)
#             elif weekday == 2 and 9 < time < 17:  # wednesday
#                 finish_timestamp = oft.replace(hour=0, minute=0, second=0) + timedelta(days=3)
#             elif weekday == 3 and 9 < time < 17:  # thursday
#                 finish_timestamp = oft.replace(hour=0, minute=0, second=0) + timedelta(days=2)
#             elif weekday == 4 and 9 < time < 17:  # friday
#                 finish_timestamp = oft.replace(hour=0, minute=0, second=0) + timedelta(days=1)
#             else:
#                 finish_timestamp = oft
#         else:
#             raise ValueError("Unknown forecast strategy: " + self.forecast)
